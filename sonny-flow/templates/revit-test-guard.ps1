...
# (trust-dialog watcher, verdict codes, Revit reuse), so block it and point at scripts/loop.ps1.
...
Sonny.Application.Tests launches a real Revit - run it through the loop gate, not raw dotnet test:
  powershell -ExecutionPolicy Bypass -File scripts\loop.ps1 [-Filter <name>] [-Final]
Read docs/architecture/revit-test-environment.md and sonny-flow rules/revit-loop.md first.
